<script>
    ...
    const loadUserData = async (slaps) => {
        ...
        slaps.forEach(slap => {
            if (slap.player) addresses.add(slap.player.toLowerCase());
            if (slap.slappee) addresses.add(slap.slappee.toLowerCase());
        });
    ...
    const displayUser = (address) => {
        if (!address) return format.address("0x0000000000000000000000000000000000000000");
    ...
    {#if !loading && pendingSlaps.length > 0}
        <Card.Root>
            ...
            <Card.Content class="overflow-x-auto">
                <div class="rounded-lg overflow-hidden border min-w-[600px] md:min-w-0">
                    <Table.Root>
                        <Table.Header>
